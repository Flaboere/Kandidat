...
public class CharacterMotor : MonoBehaviour
{
    ...
    private void UpdateFunction()
    {
		...
		// Move our character!
        movement.collisionFlags = controller.Move(currentMovementOffset); // Allocates mem? why?
    ...
    private Vector3 ApplyGravityAndJumping(Vector3 velocity)
    {
        ...
        if(grounded)
        {
            ...
            if((jumping.enabled && canControl && (Time.time - jumping.lastButtonDownTime < 0.2f)) || inAirJump)
            {
                grounded = false;
